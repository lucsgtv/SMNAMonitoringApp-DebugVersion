...
class MonitoringAppMass:
    ...
    def load_data(self):
        try:
            ...
            db_path = "C:/Users/ll7nt/SMNAMonitoringApp/examples/mass/costFile_Oper.db"
            if not os.path.exists(db_path):
                print(f"Arquivo não encontrado: {db_path}")
                return  # Saia da função se o arquivo não existir

            con = sqlite3.connect(db_path)  # Conecte-se ao banco de dados

            # Carregue os dados e certifique-se de que a coluna 'date' é convertida para datetime
            self.df = pd.read_sql_query("SELECT * FROM costCons ORDER BY date", con, parse_dates=["date"])

            # Verifique o DataFrame e os tipos de dados
            print("DataFrame carregado:")
            print(self.df.head())  # Mostre as primeiras linhas do DataFrame
            print("Tipos de dados:")
            print(self.df.dtypes)  # Mostre os tipos de dados

            # Verifique se a coluna 'date' foi carregada corretamente
            if 'date' in self.df.columns:
                print("Coluna 'date' encontrada, tentando definir como índice.")
            else:
                print("Coluna 'date' não encontrada!")

            self.df.set_index('date', inplace=True)  # Defina a coluna 'date' como índice

            # Verifique o tipo do índice após definir
            print("Tipo de índice após definir:")
            print(type(self.df.index))

            # Se o índice não é DatetimeIndex, converta manualmente
            if not isinstance(self.df.index, pd.DatetimeIndex):
                print("O índice não é DatetimeIndex, convertendo.")
                self.df.index = pd.to_datetime(self.df.index)

            # Verifique novamente
            print("Tipo de índice após conversão (se necessário):")
            print(type(self.df.index))

            self.df.replace(-1e38, np.nan, inplace=True)

            self.dc = pd.read_sql_query("SELECT * FROM costFunc ORDER BY date", con, parse_dates=["date"])
            self.dc.set_index('date', inplace=True)
            self.dc.replace(-1e38, np.nan, inplace=True)
    ...
    # Widgets do Panel
        self.Hour = pn.widgets.RadioBoxGroup(name="Hour Cycle", options=self.df.index.hour.unique().tolist())  # Use index.hour
        self.Outer = pn.widgets.RadioBoxGroup(name="Outer Loop", options=self.df.outer.unique().tolist())
        self.Vars = pn.widgets.Select(name='Variables', options=self.df.keys()[2:].tolist())
        self.use_mean = pn.widgets.Switch(name='Monthly Mean')
